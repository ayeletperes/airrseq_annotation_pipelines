...
option_list <- list(
  ...
  make_option(c("-n", "--num_mutation"), type="character", help="min number of mutaion", metavar="val")
  )
...
data <- data[data$v_mut <= num_mutation, ]
...
data <- data %>% select(sequence_id, sequence, sequence_alignment,germline_alignment,v_call, d_call, j_call, v_mut, cdr3,consensus_count, duplicate_count,v_sequence_start, v_sequence_end,v_germline_start, v_germline_end,d_sequence_start, d_sequence_end,d_germline_start, d_germline_end)
...
data <- data %>% dplyr::group_by(sequence_vdj) %>%
  dplyr::summarise(
    ...
    v_mut = v_mut[which.max(consensus_count)],
    germline_alignment = germline_alignment[which.max(consensus_count)],
    v_call = v_call[which.max(consensus_count)],
    d_call = d_call[which.max(consensus_count)],
    j_call = j_call[which.max(consensus_count)],
    cdr3 = cdr3[which.max(consensus_count)],
    v_sequence_start = v_sequence_start[which.max(consensus_count)],
    v_sequence_end = v_sequence_end[which.max(consensus_count)],
    v_germline_start = v_germline_start[which.max(consensus_count)],
