...
DATA[["sequence_v"]] <- mapply(function(seq, start, end) {
...
# Append matching reference bases to sequence_v
v_summary$sequence_v_extended <- mapply(function(seq_v, gene) {
  ...
  # Get the tail of the reference starting *after* the current sequence
  start_pos <- nchar(seq_v) + 1
  if (start_pos > nchar(ref)) {
    return(seq_v)  # no extension needed
  }

  ref_tail <- substr(ref, start_pos, nchar(ref))

  # Append the tail to the current sequence
  extended_seq <- paste0(seq_v, ref_tail)

  return(extended_seq)
}, 
v_summary$sequence_v,
v_summary$v_call)
